...
def main():
    ...
    fr = open('pid_K', 'r')
    lines = fr.readlines()
    cv2.createTrackbar('KP', 'PID_K_Controller', int(lines[0]), 500, nothing)
    cv2.createTrackbar('KI', 'PID_K_Controller', int(lines[1]), 500, nothing)
    cv2.createTrackbar('KD', 'PID_K_Controller', int(lines[2]), 500, nothing)
